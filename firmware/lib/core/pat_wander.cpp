...
// its own knob. A short fading tail shows where it's been.
#include <algorithm>
#include <cmath>
#include <cstring>
...
void render(PatternCtx& ctx) {
  ...
  if (beatTurn && beat > 0.5f && s_lastBeat <= 0.5f) s_turnPending = true;
  s_lastBeat = beat;
  ...
  for (int k = shown - 1; k >= 0; k--) {
    ...
    const int i = ctx.idx(s_trail[k][0], s_trail[k][1], s_trail[k][2]) * 3;
    buffer[i] = (uint8_t)std::lround(pr * fall);
    buffer[i + 1] = (uint8_t)std::lround(pg * fall);
    buffer[i + 2] = (uint8_t)std::lround(pb * fall);
  }
  const int hi = ctx.idx(s_x, s_y, s_z) * 3;
  buffer[hi] = (uint8_t)std::lround(cr);
  buffer[hi + 1] = (uint8_t)std::lround(cg);
  buffer[hi + 2] = (uint8_t)std::lround(cb);
}
